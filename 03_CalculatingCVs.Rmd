...
# Read in counts
```{r}
AllCounts<-read.csv("/Users/alinaklineschoder/Desktop/Data/AnalyticalValidation/Data_AnalyticalValidation_Organized/FinalCounts.csv")%>%
  dplyr::select(-c("X"))%>%
...
### Table 9. AV Repro
```{r}
Repro_Meta<-SampleInfo%>%filter(Category%in%c("BXRepeat","Repeatability"))%>%
  dplyr::select(Sample.Name,Control,Parameter)%>%
  filter(Parameter%in%c("L1R1","L1R1_5","L1R2","L1R2_5","L2R1","L2R1_5","L2R2","L2R2_5"))

Repro_Counts<-AllCounts[,Repro_Meta$Sample.Name]%>%
  rownames_to_column(var="Gene")%>%
  pivot_longer(cols=setdiff(colnames(.),"Gene"),names_to = "Sample.Name")

AVRepro<-left_join(Repro_Counts,Repro_Meta)%>%
  dplyr::select(-c(Sample.Name))%>%
  pivot_wider(names_from = Parameter,values_from = value)%>%
  rowwise()%>%
  mutate(L1R1=100*sd(c(L1R1,L1R1_5))/mean(L1R1,L1R1_5),
         L1R2=100*sd(c(L1R2,L1R2_5))/mean(L1R2,L1R2_5),
         L2R1=100*sd(c(L2R1,L2R1_5))/mean(L2R1,L2R1_5),
         L2R2=100*sd(c(L2R2,L2R2_5))/mean(L2R2,L2R2_5))%>%
  select(Gene,L1R1,L1R2,L2R1,L2R2,Control)%>%
  mutate(Type="AVRepro")
```
...
```{r}
AllTabs<-bind_rows(FT,Time,DNAse,RI,LI,Batch,Repeat,Repro,AVRepro)%>%
  pivot_longer(cols=setdiff(colnames(.),c("Gene","Control","Type")))%>%
  filter(!is.na(value))

write.csv(AllTabs,"/Users/alinaklineschoder/Desktop/Data/AnalyticalValidation/Data_AnalyticalValidation_Organized/CVs.csv")
```
